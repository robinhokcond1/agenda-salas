...
class Periods extends MY_Controller
{
	...
	function index()
	{
		...
		$this->data['title'] = 'Períodos do Dia';
	...
	function delete($id = NULL)
	{
		...
		$this->data['text'] = 'Se você excluir este período, quaisquer reservas para esse período nas <strong>all</strong> salas serão <strong>permenantly deletadas</strong>.';
	...
	function _is_after($time){
		...
		if( $end >= $start ){
			...
			$this->form_validation->set_message('_is_after', 'O horário de término deve ser igual ou superior ao horário de início de'.$this->_fix_time( $this->input->post( 'time_start' ) ).'.' );
			$ret = false;
		}
		return $ret;
	}
