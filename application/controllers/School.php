...
class School extends MY_Controller
{
	...
	*/
	...
	function details()
	{
		...
		$this->data['title'] = 'Informações da Empresa';
	...
	function details_submit()
	{
		...
		if (isset($_FILES['userfile']) && isset($_FILES['userfile']['name']) && ! empty($_FILES['userfile']['name'])) {
			...
			if ( ! $this->upload->do_upload()) {
				...
				if ($error != 'Você não selecionou um arquivo para fazer upload') {
		...
		$this->session->set_flashdata('saved', msgbox('info', 'Os detalhes da empresa foram atualizados.'));
