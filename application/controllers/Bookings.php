...
class Bookings extends MY_Controller
{
	...
	*/
		...
		switch ($style['display']) {
			...
			case 'day':
				...
				if ($this->input->post('chosen_date')) {
					...
					if (count($datearr) != 3) {
						show_error('Data escolhida, inválida.');
					}
	...
	function book()
	{
		...
		$this->data['title'] = 'Reservar uma Sala';
		...
		if (isset($query['date']) && $this->userauth->is_level(TEACHER)) {
			...
			if ($booking_status->result === FALSE) {
				...
				if ( ! $booking_status->is_future_date) {
					$msg = "A data escolhida está no passado.";
					$messages[] = msgbox('error', $msg);
				}

				if ( ! $booking_status->date_in_range) {
					$msg = "A data escolhida deve ser menor que %d dias no futuro.";
	...
	private function process_recurring()
	{
		...
		if ($errcount > 0) {
			$flashmsg = msgbox('error', 'Não foi possível efetuar uma ou mais reservas.');
		} else {
			$flashmsg = msgbox('info', 'As reservas foram criadas com sucesso.');
		}
	...
	function save()
	{
		...
		if ($this->_check_unique_booking($booking_data)) {
			...
			$flashmsg = msgbox('exclamation', "Já existe uma reserva para essa data e período.");
