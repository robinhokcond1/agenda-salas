...
class Weeks extends MY_Controller
{
	...
	function index()
	{
		...
		if ( ! $this->data['academicyear']) {
			$this->data['body'] = msgbox('warning', "Por favor, configure as semans do ano primeiro.");
		} else {
			$this->data['body'] = '';
		}
		...
		$this->data['title'] = 'Ciclo da semana do horário';
	...
	function delete($id = NULL)
	{
		...
		$this->data['text'] = 'Se você excluir esta semana, <strong>todas as reservas recorrentes</strong> anexado a esta semana não será mais visível.';

		$row = $this->weeks_model->Get($id);
		$this->data['title'] = 'Excluir semana (' . html_escape($row->name) . ')';
		$this->data['showtitle'] = $this->data['title'];
	 ...
	 function academicyear()
	 {
	 	...
	 	$this->data['title'] = 'Ano Empresarial';
	 	$this->data['showtitle'] = $this->data['title'];
	 ...
	 function saveacademicyear()
	 {
		...
		if ($this->form_validation->run() == FALSE) {
 		...
 		$this->session->set_flashdata('saved', msgbox('info', 'As datas do ano empresarial foram atualizadas.'));
